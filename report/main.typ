// ============================================================================
// CogniSOC — SOC ML Engine: Project Report
// Ankit Singh | B.Tech Cyber Security | NIET, Greater Noida
// ============================================================================
...
#set par(justify: true, leading: 0.8em, first-line-indent: 1.2em)
...
#show heading.where(level: 2): it => {
  v(0.6cm)
  block(text(size: 14pt, weight: "bold")[
    #if it.numbering != none {
      counter(heading).display(it.numbering)
      h(0.5em)
    }
...
#show heading.where(level: 3): it => {
  v(0.4cm)
  block(text(size: 12pt, weight: "bold")[
    #if it.numbering != none {
      counter(heading).display(it.numbering)
      h(0.5em)
    }
...
#align(center)[
  ...
  #v(0.5cm)
  // ── HCLTech Logo ──
  #image("images/hcltech_logo.png", width: 9cm)
  ...
  #text(size: 22pt, weight: "bold")[
    CogniSOC: AI-Powered SOC Threat \
...
#align(center)[
  Date: ________________ #h(3cm) Place: Greater Noida
]
...
// ════════════════════════════════════════════════════════════════════════════
// MAIN BODY — Reset page numbering to arabic
// ════════════════════════════════════════════════════════════════════════════
...
+ *High false-positive rates:* Generic rules generate thousands of alerts daily, most of which are benign. A 2023 study by Tines found that SOC analysts spend 32% of their time investigating alerts that turn out to be false positives [3].
+ *Inability to detect novel threats:* Signature-based systems cannot detect zero-day exploits, fileless malware, living-off-the-land (LOLBin) attacks, or sophisticated adversary techniques that do not match existing rule patterns.
+ *No behavioral context:* Individual log lines are evaluated in isolation. A single PowerShell execution is not inherently suspicious, but a burst of 50 PowerShell invocations, combined with rare process names and unusual parent-child process relationships within a 15-minute window, is a strong behavioral signal that rules alone cannot capture.
...
+ Integrates with an operational Splunk Enterprise SIEM deployment to ingest Sysmon and Suricata telemetry via the REST API.
+ Normalizes heterogeneous log formats (Sysmon XML, Suricata EVE JSON, flat Splunk key-value fields) into a unified event schema.
+ Extracts explainable behavioral features from aggregated entity-time-window observations.
+ Trains an Isolation Forest anomaly detection model on known-normal lab telemetry and scores new telemetry without retraining.
+ Correlates anomalous findings into attack-specific incidents mapped to the MITRE ATT&CK framework.
+ Automatically creates investigation cases with pre-built analyst tasks in TheHive SOAR platform.
+ Re-ingests ML findings into Splunk via HEC for unified dashboard visibility.
+ Provides real-time monitoring with auto-refreshing dashboards.

== Objectives

+ To reduce SOC analyst alert fatigue by surfacing only behaviorally anomalous activity above a configurable threshold.
+ To detect attack patterns that evade traditional rule-based detection by analyzing behavioral deviations across multiple telemetry dimensions simultaneously.
+ To provide explainable anomaly scores with human-readable reason strings, risk factors, and MITRE ATT&CK context so that analysts understand *why* an alert was raised.
+ To automate the incident response workflow by creating structured TheHive cases with prioritized tasks.
+ To validate the system against real attack simulations using the Atomic Red Team framework.
...
This report is organized into ten chapters. @chap-litreview presents a literature review of existing SOC architectures, machine learning approaches for anomaly detection, and the MITRE ATT&CK framework. @chap-architecture describes the system architecture, including the 4-machine lab setup, technology stack, and data flow. @chap-implementation details the implementation of each software module with code excerpts and configuration. @chap-features explains the feature engineering and machine learning model design. @chap-correlation covers the correlation engine, incident classifier, and priority scoring. @chap-dashboard presents the dashboard and visualization layer. @chap-testing documents the testing methodology and results using Atomic Red Team. @chap-problems discusses challenges encountered during development and their solutions. @chap-conclusion provides conclusions and future work.
...
+ *Zero-cost deployment:* The entire system was built using open-source and free-tier tools (Splunk Free License, TheHive, Sysmon, Suricata, scikit-learn), demonstrating that advanced SOC capabilities are accessible to educational institutions and small organizations without commercial license costs.
