...
+ Train the Isolation Forest baseline on 2 hours of normal (benign) activity.
...
In this paper, we introduced CogniSOC, an open-source behavioral analytics framework designed to bridge the critical gap between academic anomaly detection models and operational SOC workflows. While significant research exists on ML algorithms for cybersecurity, few studies address the end-to-end integration required to make these algorithms actionable. 

By integrating an Isolation Forest model with SIEM telemetry, a MITRE ATT&CK correlation engine, and SOAR automation, CogniSOC provides a complete reference architecture. Our extensive evaluation yielded several key outcomes:
1. *Quadrupled Detection Recall:* The ML model improved attack detection from 10.7% (Rules-Only) to 42.8%, successfully catching evasive behaviors.
2. *Effective Alert Volume Reduction:* By correlating raw anomalies, the system reduced 9 discrete findings down to 3 cohesive, actionable incidents.
3. *Automated Prioritization:* Incidents were automatically triaged into P1 and P2 priorities, streamlining the incident response process for human analysts.

CogniSOC successfully condenses raw behavioral anomalies into cohesive, prioritized incidents, proving that ML models can significantly reduce alert fatigue when properly integrated into the SOC pipeline.
...
We evaluated CogniSOC in our simulated SOC lab environment using the Atomic Red Team framework. To quantify the system's operational value, we conducted a baseline comparison against a traditional Rules-Only detection approach and performed an ablation study to isolate the contribution of each pipeline stage.

== Experimental Setup
We generated synthetic telemetry modeled after real-world Sysmon and Suricata events. The dataset comprised 61 time windows of 15 minutes each: 40 benign baseline windows and 21 malicious windows containing simulated attacks across seven distinct MITRE ATT&CK techniques (e.g., encoded PowerShell execution, LOLBin proxy execution, and network port scanning).

For our baseline, we implemented a Rules-Only detector that applied 13 standard Sigma rules directly to the feature windows without any machine learning intervention. We then evaluated CogniSOC across three configurations:
- *Config A (Isolation Forest Only):* Findings generated solely by the ML model with a score > 90.
- *Config B (IF + Correlation):* ML findings aggregated into incidents by the correlation engine.
- *Config C (Full System):* Correlated incidents filtered and scored by the prioritization module.

== Baseline Comparison: Detection Performance

#figure(
  image("images/evaluation_figures/performance_comparison.png", width: 100%),
  caption: [Performance Comparison: Rules-Only Baseline vs. CogniSOC (Isolation Forest)]
)

As shown in the performance comparison figure, the traditional Rules-Only baseline achieved a high precision (100%) but a very poor recall of 10.7%, successfully identifying only 3 out of the 21 attack windows. This demonstrates the primary weakness of rigid, signature-based rules: they frequently fail to detect variations in attack techniques or slightly obfuscated behaviors.

In contrast, the Isolation Forest model (Config A) quadrupled the recall to 42.8% (detecting 9 out of 21 attack windows) while maintaining 100% precision. By learning the normal behavioral baseline of the environment, the ML model successfully identified anomalous patterns---such as bursts of PowerShell execution coupled with rare processes---that bypassed the static rules.

== Alert Volume Reduction

While the Isolation Forest improved detection rates, relying solely on ML scores introduces a new challenge: an increase in raw alert volume. The Rules-Only baseline generated 3 alerts, whereas the raw ML model generated 9 suspicious findings. If sent directly to analysts, this would increase alert fatigue.
...
This highlights the critical importance of CogniSOC's subsequent pipeline stages. When the 9 anomalies were processed by the Correlation Engine (Config B), they were intelligently grouped based on temporal proximity and behavioral similarity into just 3 cohesive incidents. The Full System (Config C) maintained these 3 incidents, prioritizing them for investigation.

This multi-stage approach ensures that while the machine learning layer increases the detection recall, the correlation and prioritization layers prevent an explosion in alert volume. It bridges the gap between raw algorithmic output and actionable SOC intelligence.
